...
func packageCacheFile(cacheDir, absPackagePath, checksum string) string {
	return filepath.Join(cacheDir, absPackagePath, checksum)
}
...
func build(packagePath string, absOutputPath string, compilerFlags []string, compilerEnv map[string]string) bool {
	compileCmd := exec.Command("go", "build", "-trimpath", "-buildvcs=false", "-o", absOutputPath)
	...
	compileCmd.Stderr = os.Stderr // TODO (misha): It would be nice to add color to this output
...
func updateCache(cacheDir, absPackagePath, sourceChecksum string, compilerFlags []string, compilerEnv map[string]string) (retUpdated bool, _ error) {
	// Lock the package directory
	p := filepath.Join(cacheDir, absPackagePath)
	...
	return build(absPackagePath, packageCacheFile(cacheDir, absPackagePath, sourceChecksum), compilerFlags, compilerEnv), nil
}
